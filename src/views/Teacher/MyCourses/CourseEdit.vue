...
export default {
  ...
  setup() {
    ...
    const loadVideos = async () => { try { loadingVideos.value = true; const res = await axios.get(`${base}/course/video/list`, { params: { courseId, _t: Date.now() }, headers: token.value ? { Authorization: `Bearer ${token.value}` } : {} }); const body = res?.data; videos.value = (body && Number(body.code) === 200 && Array.isArray(body.data)) ? body.data : []; if (Array.isArray(videos.value) && videos.value.length > 0 && (video.value.videoId == null || video.value.videoId === '')) { const v0 = videos.value[0]; video.value.videoId = v0.videoId || v0.id; video.value.videoTitle = v0.videoTitle || v0.title || ''; video.value.duration = v0.duration || null } } catch { videos.value = [] } finally { loadingVideos.value = false } }
    ...
    const loadDocs = async () => { try { loadingDocs.value = true; const res = await axios.get(`${base}/course/document/list`, { params: { courseId }, headers: token.value ? { Authorization: `Bearer ${token.value}` } : {} }); const body = res?.data; documents.value = (body && Number(body.code) === 200 && Array.isArray(body.data)) ? body.data : [] } catch { documents.value = [] } finally { loadingDocs.value = false } }
