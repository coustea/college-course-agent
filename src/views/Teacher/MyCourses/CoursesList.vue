      ...
      <div class="course-card" v-for="course in paginatedCourses" :key="course.id">
        ...
        <div class="course-content">
          ...
          <div class="teacher-actions">
            ...
            <el-button size="small" plain @click="openVideoPicker(course)">
              <el-icon>
                <VideoPlay/>
              </el-icon>
              观看视频
            </el-button>
            <el-button size="small" plain @click="previewCourseDocs(course)">
              <el-icon>
                <Document/>
              </el-icon>
              查看文档
    ...
    <el-dialog v-model="videoPickerVisible" :title="videoPickerTitle" width="560px">
      <el-table :data="videoPickerList" style="width: 100%" size="small" height="360">
        <el-table-column label="#" width="60">
          <template #default="scope">{{ (scope.row.videoIndex ?? scope.$index) + 1 }}</template>
        </el-table-column>
        <el-table-column prop="videoTitle" label="标题" min-width="240" />
        <el-table-column prop="duration" label="时长(秒)" width="100" />
        <el-table-column label="操作" width="120" fixed="right">
          <template #default="scope">
            <el-button size="small" type="primary" @click="openVideoAt(videoPickerCourse, scope.$index)">播放</el-button>
          </template>
        </el-table-column>
      </el-table>
      <template #footer>
...
export default {
  ...
  components: {
    ...
    Notebook,
    Loading,
    CoursePlayer: () => import('@/components/CoursePlayer.vue'),
  ...
  setup() {
      ...
      try {
        ...
        if (body && Number(body.code) === 200 && Array.isArray(body.data)) {
          courses.value = body.data.map(c => ({
            ...
            status: 'published',
            categoryId: null,
            createTime: c.startDate ?? null,
            studentCount: 0,
            videoCount: Array.isArray(c.videos) ? c.videos.length : (c.videoCount ?? 0) ?? 0,
            documentCount: Array.isArray(c.documents) ? c.documents.length : (c.documentCount ?? 0) ?? 0,
            completionRate: 0,
          }))
          try { await updateStudentCounts() } catch {}
        } else {
          courses.value = []
          ElMessage.error(body?.message || '加载课程失败')
        }
      } catch (error) {
    ...
    // 动态补齐每门课程的学生数量：/api/teacher/enrollments/students?courseId=...
        ...
        workers.push((async () => {
          while (true) {
            ...
            try {
              ...
              courses.value[current] = { ...courses.value[current], studentCount: 0 }
            }
          }
        })())
      }
    ...
    // 规范化资源 URL（在 4173 下为 /uploads/** 补后端主机）
    const normalizeUrl = (url) => {
      if (!url || typeof url !== 'string') return ''
      if (/^https?:\/\//i.test(url) || url.startsWith('data:')) return encodeURI(url)
      if (url.startsWith('/uploads/')) return encodeURI((backendHost || '') + url)
      return encodeURI(url)
    }

    // ========== 课程视频/文档预览（对齐旧版逻辑：带缓存） ==========
    ...
    const loadCourseVideos = async (courseId) => {
      try {
        const base = import.meta?.env?.VITE_API_BASE_URL || 'http://localhost:9999/api'
        const headers = token.value ? { Authorization: `Bearer ${token.value}` } : {}
        const res = await axios.get(`${base}/course/video/list`, { params: { courseId }, headers })
        const body = res?.data
        if (body && Number(body.code) === 200 && Array.isArray(body.data)) return body.data
        return []
      } catch { return [] }
    }
    const getCourseVideos = async (courseId) => {
      const key = String(courseId)
      const cached = courseVideosCache.value[key]
      if (cached && Array.isArray(cached)) return cached
      const list = await loadCourseVideos(courseId)
      courseVideosCache.value[key] = list
      return list
    }
    const loadCourseDocs = async (courseId) => {
      try {
        const base = import.meta?.env?.VITE_API_BASE_URL || 'http://localhost:9999/api'
        const headers = token.value ? { Authorization: `Bearer ${token.value}` } : {}
        const res = await axios.get(`${base}/course/document/list`, { params: { courseId }, headers })
        const body = res?.data
        if (body && Number(body.code) === 200 && Array.isArray(body.data)) return body.data
        return []
      } catch { return [] }
    ...
    const openVideoPicker = async (course) => {
      ...
      if (!cid) { ElMessage.error('缺少课程ID'); return }
      const list = await getCourseVideos(cid)
      if (!list.length) { ElMessage.info('该课程暂无视频'); return }
      videoPickerCourse.value = course
      videoPickerTitle.value = (course.title || course.courseName || '课程') + ' - 选择视频'
      videoPickerList.value = list
      videoPickerVisible.value = true
    }
    const openVideoAt = async (course, index) => {
      const cid = course.id || course.courseId
      const list = await getCourseVideos(cid)
      if (!list.length) { ElMessage.info('该课程暂无视频'); return }
      playerCourseId.value = cid
      playerTitle.value = course.title || course.courseName || '课程视频'
      playerChapters.value = list.map(v => ({
        title: v.videoTitle || `第${(v.videoIndex ?? 0) + 1}节`,
        videoUrl: normalizeUrl(v.videoUrl),
        duration: v.duration ? String(v.duration) : ''
      }))
      playerStartIndex.value = Math.max(0, Math.min(index || 0, list.length - 1))
      videoPickerVisible.value = false
      playerVisible.value = true
    }

    const previewCourseVideos = async (course) => {
      const cid = course.id || course.courseId
      if (!cid) { ElMessage.error('缺少课程ID'); return }
      const list = await getCourseVideos(cid)
      if (!list.length) {
        ElMessage.info('该课程暂无视频')
        // 回退：若课程卡片本身有 resourceUrl 且是视频，直接播放
        const url = (course.resourceUrl || course.image || '').toString()
        if (/\.(mp4|webm|mov)(\?.*)?$/i.test(url)) {
          playerCourseId.value = cid
          playerTitle.value = course.title || '课程视频'
          playerChapters.value = [{ title: course.title || '视频', videoUrl: normalizeUrl(url), duration: '' }]
    ...
    const previewCourseDocs = async (course) => {
      const cid = course.id || course.courseId
      if (!cid) { ElMessage.error('缺少课程ID'); return }
      const list = await loadCourseDocs(cid)
      if (!list.length) {
        ElMessage.info('该课程暂无文档')
        // 回退：若课程卡片 resourceUrl 是文档，直接预览
        const url = (course.resourceUrl || course.image || '').toString()
        if (/\.(pdf|docx?|pptx?)(\?.*)?$/i.test(url)) {
          docTitle.value = course.title || '课程文档'
          docFileUrl.value = normalizeUrl(url)
      ...
      const d = list[0]
      docTitle.value = d.title || d.documentTitle || '课程文档'
      docFileUrl.value = d.fileUrl ? normalizeUrl(d.fileUrl) : ''
      docHtmlContent.value = d.content || ''
      docProgress.value = 0
      docId.value = d.id || null
      docImage.value = ''
      docDuration.value = ''
      docVisible.value = true
    }
    ...
    const deleteCourse = async (course) => {
      try {
        ElMessageBox.confirm(
          ...
          const base = (import.meta?.env?.VITE_API_BASE_URL || 'http://localhost:9999/api')
          const response = await axios.delete(`${base}/course/delete`, {
